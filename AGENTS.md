...
- `serviceId` is this repo's name with the hyphen removed — `neo-01` → `neo01`.
...
Pushing to `main` publishes `ghcr.io/neueda-learning/neo-01-{backend,frontend}` and **this repo
...
- **The front end's path prefix is baked at build time.** `APP_BASE_PATH=/neo-01` is a
...
- **`backend/` and `frontend/` stay at the repo root** — the system compose builds them by
  path (`./neo-01/backend`). Moving them breaks the whole stack.
